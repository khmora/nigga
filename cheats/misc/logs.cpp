...
void eventlogs::paint_traverse( ) {
	...
	auto last_y = 6;
	const auto font = fonts[ ValiumTEXT ];
	const auto name_font = fonts[ ValiumBOLD ];
	auto name_size = render::get( ).text_width( name_font, "[Valium]" );
	...
	for ( unsigned int i = 0; i < logs.size( ); i++ ) {
		...
		render::get( ).text( name_font, log.x, last_y + log.y, front_color, HFONT_CENTERED_NONE, "[Valium]" );
