...
void worldesp::grenades( weapon_t * m_nade, Box box ) {
	...
	render::get( ).text( fonts[ ValiumTEXT ], box.x + ( box.w / 2 ), box.y, color, HFONT_CENTERED_X, entity_name.c_str( ) );
}
...
void worldesp::weapons( weapon_t * e, Box box ) {
	...
	render::get( ).text( fonts[ ValiumTEXT ], box.x + ( box.w / 2 ), box.y, color, HFONT_CENTERED_X, name );
}
